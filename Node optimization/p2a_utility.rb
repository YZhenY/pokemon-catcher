...
# Ensure that this file is in the same folder as p2a_main.rb
...
def no_error(selected_stops, pop_map)
  if selected_stops == nil
    ...
    return false
  elsif selected_stops.count<1 or selected_stops.count>pop_map.flatten.size
    puts "Your method returned #{selected_stops.count} stops. It should be at least 1, and smaller than the size of the pop map (#{pop_map.flatten.size})."
    puts "Error : your method returned an invalid number of stops."
    return false  
...
# returns score of selected stops
# use true for verbose if you want more info printed out
def calculate_score_for_map(pop_map, cost_map, stops, verbose=false)

  # (1) calculate distance cost. as for project 1
  dist_score = 0.0
  for y in 0...pop_map.length
    for x in 0...pop_map[0].length
      ...
      dist_score = dist_score + cell_score
    end
  end
  
  # (2) calculate cost of all stop positions
  ...
  for i in 0...stops.length
	...
	if verbose
	  puts "stop at [#{current_stop[0]},  #{current_stop[1]}] costs :#{cost_of_curr_position}"
	end
	
  end
  
  final_total_cost = dist_score + cost_of_positions
  
  if verbose
    puts
    puts "cost of stops (based on cost map): #{cost_of_positions.round}"  
    puts "distance costs (based on pop map): #{dist_score.round}"
    puts "final total cost                 : #{final_total_cost.round}"
  end
